...
export class EventService{
    ...
    //---------------------------------- Update ------------------------------
    updateEvent (codigo_tipo_evento:number,nombre:string,lugar:string,fecha_hora:string,numero_maximo_acompanantes:number,descripcion:string) : Observable<Event>  {
      let body = JSON.stringify({nombre,lugar,fecha_hora,numero_maximo_acompanantes,descripcion,codigo_tipo_evento});
      let headers = new Headers({ 'Content-Type': 'application/json' });
      let options = new RequestOptions({ headers: headers });
