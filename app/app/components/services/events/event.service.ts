...
export class EventService{
    ...
    getInvitedEventsbyUser(pUser:number){
      return this.http.get(httpConfig.host+"eventos/invitacion_usuario/"+pUser)
                      .map(res => res.json())
                      .catch(this.handleError);
    }
    getConfirmedEventsbyUser(pUser:number){
      return this.http.get(httpConfig.host+"eventos/confirmacion_usuario/"+pUser)
                      .map(res => res.json())
                      .catch(this.handleError);
    }
    getFinalizedEventsbyUser(pUser:number){
      return this.http.get(httpConfig.host+"eventos/finalizados_usuario/"+pUser)
                      .map(res => res.json())
                      .catch(this.handleError);
    }
    ...
    getInvitedUser(codigo_usuario:number, codigo_evento:number){
      return this.http.get(httpConfig.host+"usuarios_invitados/"+codigo_usuario+"-"+codigo_evento)
                      .map(res => res.json())
                      .catch(this.handleError);
    }
    ...
    updateInvitedUser(codigo_evento:number, codigo_usuario:number, confirmado:boolean, precio_entradas:number){
      let body = JSON.stringify({codigo_evento,codigo_usuario,confirmado,precio_entradas});
      let headers = new Headers({ 'Content-Type': 'application/json' });
      let options = new RequestOptions({ headers: headers });
      return this.http.put(httpConfig.host+"usuarios_invitados/"+body, body, options)
